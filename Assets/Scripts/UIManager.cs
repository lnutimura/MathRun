...
public class UIManager : MonoBehaviour {
    ...
    public void SaveLevel () {
		...
		if (string.IsNullOrEmpty(m_levelInputField.text)) {
            ...
            MenuManager.FeedBackError(m_dialogLabel, "Digite o nome do nível.");
            return;
		}
    ...
    IEnumerator ISaveWWWForm (WWWForm wwwForm) {
		...
		if (www.error == null) {
			if (www.text == "1") {
				Debug.Log("Salvou!");
                MenuManager.FeedBackOk(m_dialogLabel, "Salvou nível com sucesso!");
            } else {
				Debug.Log("Não salvou: " + www.text);
                MenuManager.FeedBackError(m_dialogLabel, "Não foi possível salvar o nível");
            }
		} else {
			Debug.Log("Erro de conexão: " + www.error);
            MenuManager.FeedBackError(m_dialogLabel, "Erro de conexão.");
        }
    }
